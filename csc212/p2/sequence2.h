...
// There is no implementation file provided for this class since it is an exercise from Chapter 4 of "Data Structures and Other Objects Using C++"
// TYPEDEFS and MEMBER CONSTANTS for the sequence class:
//   typedef ____ value_type
//    sequence::value_type is the data type of the items in the sequence. It may be any of the C++ built-in types (int, char, etc.), or a class with a default constructor, an assignment operator, and a copy constructor.
...
//     sequence::DEFAULT_CAPACITY is the initial capacity of a sequence that is created by the default constructor.
// CONSTRUCTOR for the sequence class:
//   sequence(size_t initial_capacity = DEFAULT_CAPACITY)
//     Postcondition: The sequence has been initialized as an empty sequence.
//     The insert/attach functions will work efficiently (without allocating new memory) until this capacity is reached.
// MODIFICATION MEMBER FUNCTIONS for the sequence class:
// void resize(size_type new_capacity)
//Postcondition: The sequence's current capacity is changed to the new_capacity (but not less that the number of items already on the list). The insert/attach functions will work efficiently (without allocating new memory) until this new capacity is reached.
//
//   void start( )
...
//   If there is insufficient dynamic memory, then the following functions throw a BAD_ALLOC exception: The constructors, insert, attach.
